...
## nominations for AF -> [97 (1) ,80(2), 72 (1), 49(1), 44(1), 42(1), 29(2), 25(1)]
dataset = 'ludb'
w1_size = 0.030
success = 0
number_of_p_dots = 0
for i in range(1, 201, 1):
    ...
    for seg in range(0,1):#ecg_dict_copy['num_of_segments']):
        ...
        q_ann, _, s_ann = qrs.find_q_r_s_ann(ecg_dict_original, seg, True, False, True, realLabels=True)
        if q_ann.size == 0 or s_ann.size == 0:
            continue
        p_real_peaks = p_wave_detection.p_peaks_annotations(ecg_dict_original, 'real', seg)
